...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx: Any, x, weight):
        ...
        ctx.D_TILE_SIZE = max(ctx.D_TILE_SIZE, 16)  # Ensure minimum tile size
        ctx.ROWS_TILE_SIZE = 16  # Each thread processes 16 batch elements at a time
        ctx.input_shape = input_shape

        # Launch our kernel with n instances in our 1D grid.
        n_rows = x.shape[0]  # Number of rows after flattening

        # Need to initialize empty result tensor as 1D (kernel expects flat output)
        ...
        weighted_sum_fwd[(cdiv(n_rows, ctx.ROWS_TILE_SIZE),)](
            ...
            x.stride(1),
            weight.stride(0),
            y.stride(0),  # Now this is 1, as expected
            ROWS=n_rows,
            D=D,
